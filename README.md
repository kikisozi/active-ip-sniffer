...
## v3.6.1：CFCDN 保留 + 全部可用 IP 导出

- 保留/恢复线上 3.6.0 的 **CFCDN VLESS** 管理能力：Cloudflare 托管 A 记录、VLESS 链接、候选 IP、延迟阈值、启停、立即检查与自动恢复切换均持久化。
- CFCDN 默认每 10 分钟检查；当前节点异常时按 0.5 MiB / 10 秒真连接验证候选，默认连续 3 轮、轮间 60 秒，只有全部轮次通过的候选才允许切换。
- 兼容保留 3.6.0 配置中的 `saved_vless` 与 CFCDN 历史状态字段，升级不会因为再次保存设置而丢失。
- 同时包含 v3.5.3 的 CF Direct 改进：页面仍展示 Top 20，但可选择导出 **全部可用 IP**。
