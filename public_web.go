...
func (a *app) publicRoutes() http.Handler {
	...
	mux.HandleFunc("/api/candidates", func(w http.ResponseWriter, r *http.Request) {
		...
		writeJSON(w, http.StatusOK, map[string]any{"items": a.public.candidates(), "precision_mb": publicPrecisionMB, "quick_mb": 1, "quick_timeout_s": 2})
	})
...
<div class="card"><h2>优选 IP 用户本地测速</h2><p>该页面把候选 IP 交给你电脑上的 <code>v probe</code>，真正的 TCP/TLS/下载流量从你的网络出口发出。浏览器本身不能把 TLS 的 SNI/Host 强制连接到任意候选 IP，因此准确模式需要本地探针。</p><div class="row"><input id="token" type="password" placeholder="Local probe token" style="min-width:300px"><select id="egress"><option value="direct">Direct 本地出口</option><option value="warp">WARP Local Proxy</option></select><input id="proxy" value="127.0.0.1:40000" placeholder="WARP SOCKS5 地址"><button id="connect">连接探针</button></div><p id="state" class="note">先在本机执行 <code>v probe</code>，再粘贴 Token。</p></div>
...
const $=s=>document.querySelector(s),probe='http://127.0.0.1:18767';let token='',candidates=[],job='',probeEgress={};
